...
public class WalaUtils {
    ...
    /**
     * Recursively checks whether the app loader is within the application scope
     *
     * @param loader
     * @return
     */
    private static boolean isApplicationClassLoader(IClassLoader loader) {
        IClassLoader next = loader;
        while (next != null) {
            if (next.getReference().equals(ClassLoaderReference.Application))
                return true;
            next = loader.getParent();
        }
        return false;
    }
    ...
    public static boolean isApplicationScope(IClass iClass) {
        return iClass != null && isApplicationClassLoader(iClass.getClassLoader());
    }
    ...
    public static boolean isApplicationScope(CGNode node) {
        return isApplicationClassLoader(node.getMethod().getDeclaringClass().getClassLoader());
    }
    ...
    public static boolean isApplicationScope(Statement s) {
        return isApplicationClassLoader(s.getNode().getMethod().getDeclaringClass().getClassLoader());
    }
...
//        return isApplicationClassLoader(invokeInst.getDeclaredTarget().getDeclaringClass().getClassLoader());
//    }
